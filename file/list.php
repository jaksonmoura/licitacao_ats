    ...
    if (isset($_GET['date'])) {
        ...
        $list = $link->query("SELECT * from files where month(created_at) = month('$date') and year(created_at) = year('$date') order by created_at ASC;");
		  ...
		  $list = $link->query("SELECT * from files order by created_at ASC;");
    }
      ...
      <tbody>
        ...
        <?php 
          while ($f = mysqli_fetch_assoc($list)){
            ...
            echo '<td>';
            if (LOGGED) {
              echo '<a href="#"><img src="../assets/img/remove.png" alt="Remover" title="Remover"></a>
                    <a href="#"><img src="../assets/img/edit.png" alt="Editar" title="Editar"></a>';
            }
